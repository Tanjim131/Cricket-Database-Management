...
public class ThirdInningsBowlingController implements Initializable {
    @FXML
    ...
    private void populateComboBox(ComboBox <String> comboBox){
        ...
        try{
            plSqlQuery.getCst().setString(1, Insert2ndInningsController.s_1_InningsID);
    ...
    void nextPage(ActionEvent event) {
        ...
        if(result.isPresent() && result.get().getText().equals("Yes")){
            insertIntoDatabase();
            loadFXML("databaseBackEnd/inningsPlayer","FourthInningsBatting");
        }
    }
}
